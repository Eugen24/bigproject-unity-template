...
namespace CodeStage.AntiCheat.Editor
{
	internal class ActPostprocessor:AssetPostprocessor
	{
		...
		private static void OnPostprocessAllAssets(String[] mportedAssets, String[] deletedAssets, String[] movedAssets, String[] movedFromAssetPaths)
		{
			...
			if (deletedAssets.Length > 0)
			{
				foreach (string deletedAsset in deletedAssets)
				{
					...
					{
#if (DEBUG || DEBUG_VERBOSE || DEBUG_PARANIOD)
						Debug.LogWarning("Looks like Injection Detector data file was accidentally removed! Re-creating...\nIf you wish to remove " + ActEditorGlobalStuff.INJECTION_DATA_FILE + " file, just disable Injection Detecotr in the ACTk Options window.");
#endif
		...
		internal static void InjectionAssembliesScan(bool forced)
		{
			...
			sw.Stop();
			 Debug.Log("[ACTk] Injection Detector Assemblies Scan\n");
			 Debug.Log("[ACTk] Paths:\n" +
...
#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Looking for all assemblies in current project...");
			sw.Start();
#endif
			allLibraries.Clear();
...
#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Total libraries found: " + allLibraries.Count);
			sw.Start();
#endif
...
#if (DEBUG_PARANIOD)
				sw.Stop();
				 Debug.Log("[ACTk] Checking library at the path: " + libraryPathLowerCase);
				sw.Start();
#endif
...
#if (DEBUG || DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			string trace = "[ACTk] Found assemblies (" + allowedAssemblies.Count + "):\n";
...
#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
				sw.Stop();
				 Debug.Log("[ACTk] Creating resources folder: " + ActEditorGlobalStuff.RESOURCES_PATH);
				sw.Start();
#endif
...
#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Processing default whitelist");
			sw.Start();
#endif
...
#endif
				bw.Close();
				Debug.LogError("[ACTk] Can't find " + ActEditorGlobalStuff.INJECTION_DEFAULT_WHITELIST_FILE + " file!\nPlease, report to " + ActEditorGlobalStuff.REPORT_EMAIL);
				return;
			}

#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Processing user whitelist");
			sw.Start();
#endif
...
#if (DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Processing project assemblies");
			sw.Start();
#endif
...
#if (DEBUG || DEBUG_VERBOSE || DEBUG_PARANIOD)
			sw.Stop();
			 Debug.Log("[ACTk] Assemblies scan duration: " + sw.ElapsedMilliseconds + " ms.");
#endif

			if (allowedAssembliesCount == 0)
			{
				Debug.LogError("[ACTk] Can't find any assemblies!\nPlease, report to " + ActEditorGlobalStuff.REPORT_EMAIL);
			}
		...
		private static bool InjectionDetectorTargetCompatibleCheck()
		{
			...
			{
				Debug.LogWarning("[ACTk] Injection Detector is not available on selected platform (" + EditorUserBuildSettings.activeBuildTarget + ") and will be disabled!");
